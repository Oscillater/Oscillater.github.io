...
export default function HomepageFeatures(): JSX.Element {
  ...
  const { ref: featuresRef, isVisible: featuresVisible } = useIntersectionObserver({ threshold: 0.1 });
  const { ref: aboutRef, isVisible: aboutVisible } = useIntersectionObserver({ threshold: 0.1 });
  ...
  return (
    <section ref={featuresRef} className={styles.features}>
      ...
      {/* 统计区域 */}
      <StatsSection timeData={timeData} wordStats={wordStats} isVisible={featuresVisible} />

      <div ref={aboutRef}>
        <motion.div
          className="container"
          initial={{ opacity: 0, y: 40 }}
          animate={aboutVisible ? { opacity: 1, y: 0 } : { opacity: 0, y: 40 }}
